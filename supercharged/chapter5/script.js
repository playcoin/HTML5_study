...
var DHTMLSprite = function(params){
	...
	var $element = params.$drawTarget.append('<div style="display:none"/>').find(':last'),
...
// Game-wide collision manageer with broad-phrase collision check
var collisionManager = function () {
	...
	return {
		newCollider: function(colliderFlag, collideeFlags, width, height, callback) {
			var list, indexStr = '' + listIndex++,	// list is a ID to colliderObj map
				checkIndex; 	// listIndexStr and checkIndex give ID key to each colliderObj.
			var colliderObj = {
				halfWidth : width / 2,
				halfHeight : height / 2,
				centerX : 0,
	...
	that = {
		...
		move: function () {
			...
			if (moveIndex === -1) {
				...
				if (!waitFire) {
					var fireAlien = aliensFireList[Math.floor(Math.random() * (aliensFireList.length))];
...
var laser = function (x, y, callback) {
	...
	var collider = SYS_collisionManager.newCollider(LASER, 
		...
		gameCallback = function (messageObj) {
			...
			switch (messageObj.message) {
				...
				case 'saucerHit':
					...
					score += pts * 50;
					updateScores();
					animEffect(messageObj.x, messageObj.y, [pts + 20], 250);
					break;
				case 'playerKilled':
